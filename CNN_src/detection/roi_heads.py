...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            topk_idxs.append(keep)
        return all_boxes, all_scores, all_labels, topk_idxs
    ...
    def forward(
        ...
        if self.training:
            ...
            boxes, scores, labels, topk_idxs = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
        ...
        if self.has_gcs_branch:
            #adding code for gcs-score branch
            boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]
            box_features_batch_wise = box_features.split(boxes_per_image, 0)
            if self.training:
                num_images = len(proposals)
                box_features_pos_batch_wise = []
                matched_idxs_pos = []
                for img_id in range(num_images):
                    pos = torch.where(labels[img_id] > 0)[0]
                    box_features_pos_batch_wise.append(box_features_batch_wise[img_id][pos])
                    matched_idxs_pos.append(matched_idxs[img_id][pos])
                box_features_pos = torch.cat(box_features_pos_batch_wise,0)
            else:
                box_features_topk = []
                for idx,keep in enumerate(topk_idxs):
                    box_features_topk.append(box_features_batch_wise[idx][keep])
                box_features_pos = torch.cat(box_features_topk,0)

            # gcs prediction forward pass
            gcs_scores = self.gcs_predictor(box_features_pos)[:,0]

            # loss / inference
            if self.training:
                if targets is None:
                    raise ValueError("targets cannot be None when training")
                gt_gcs_scores = [t["gcs_scores"] for t in targets]
                matched_gt_gcs_scores = []
                for img_id in range(num_images):
                    gt_gcs_scores_in_image = gt_gcs_scores[img_id]
                    if gt_gcs_scores_in_image.numel() == 0:
                        gt_gcs_scores_in_image = torch.zeros((1, 1), dtype=dtype, device=device)
                    matched_gt_gcs_scores.append(gt_gcs_scores_in_image[matched_idxs_pos[img_id]])
                matched_gt_gcs_scores = torch.cat(matched_gt_gcs_scores,0)
                gcs_loss = self.gcs_loss(gcs_scores,matched_gt_gcs_scores)
                loss_gcs = {"loss_gcs": gcs_loss}
                losses.update(loss_gcs)
            else:
                # labels = [r["labels"] for r in result]
                pos_boxes_per_image = [labels_per_image.shape[0] for labels_per_image in labels]
                gcs_scores_batch_wise = gcs_scores.split(pos_boxes_per_image, 0)
                for gcs, r in zip(gcs_scores_batch_wise, result):
                        r["gcs"] = gcs
