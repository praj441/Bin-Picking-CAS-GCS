...
class BinDataset(torch.utils.data.Dataset):
    def __init__(self, root, transforms, type, label_issue=False, real_noise=False, train=True,eval_data_type = 'ours-sim'):
        self.root = root
    ...
    def __getitem__(self, idx):
        # print(self.dps[idx],self.imgs[idx],self.masks[idx],self.dmaps[idx])
        if self.type == 'LDD':
