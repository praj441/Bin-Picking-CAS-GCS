...
class Compose:
	...
	def __call__(self, image, target, aux_target):
		for t in self.transforms:
			# print(t)
			image, target , aux_target= t(image, target, aux_target)
...
# 	def forward(
# 		self, image: Tensor, target: Optional[Dict[str, Tensor]] = None, aux_target:Optional[Tensor]=None
# 	) -> Tuple[Tensor, Optional[Dict[str, Tensor]],Optional[Tensor]]:
...
class FixedSizeCrop(nn.Module):
	...
	def _pad(self, img, target, depth_img, padding):
		...
		if depth_img is not None:
			depth_img = F.pad(depth_img, padding, self.fill, self.padding_mode)
		...
		return img, target, depth_img

	def _crop(self, img, target, depth_img,  top, left, height, width):
		img = F.crop(img, top, left, height, width)
		if depth_img is not None:
			depth_img = F.crop(depth_img, top, left, height, width)
		if target is not None:
			boxes = target["boxes"]
			boxes[:, 0::2] -= left
			boxes[:, 1::2] -= top
	...
	def forward(self, img, target=None,depth_img=None):
		...
		if new_height != height or new_width != width:
			...
			img, target, depth_img = self._crop(img, target, depth_img, top, left, new_height, new_width)

		pad_bottom = max(self.crop_height - new_height, 0)
		pad_right = max(self.crop_width - new_width, 0)
		if pad_bottom != 0 or pad_right != 0:
			img, target, depth_img = self._pad(img, target,depth_img, [0, 0, pad_right, pad_bottom])
...
class ToTensor(nn.Module):
	def forward(
		...
		image =  F.to_tensor(image).type(torch.float)
		return image, target, aux_target
...
class ScaleJitter(nn.Module):
	...
	def forward(
		...
		aux_target = F.resize(aux_target, [new_height, new_width], interpolation=self.interpolation)
...
class PILToTensor(nn.Module):
	def forward(
		...
		image = F.pil_to_tensor(image).type(torch.float)
		return image, target, aux_target
...
class Resize(torch.nn.Module):
	...
	def __init__(self, size, interpolation=InterpolationMode.BILINEAR, max_size=None, antialias=False):
		super().__init__()
	...
	def forward(self, img, target=None, depth_img=None):
		...
		resized_img = F.resize(img, self.size, self.interpolation, self.max_size, self.antialias)
		
		depth_img = depth_img.unsqueeze(0)
		resized_depth_img = F.resize(depth_img, self.size, self.interpolation, self.max_size, self.antialias)
		resized_depth_img = resized_depth_img.squeeze(0)

		#resize masks
		if "masks" in target:
			mask = target["masks"]
			mask = torch.nn.functional.interpolate(mask[:, None].float(), size=self.size, scale_factor=None, recompute_scale_factor=None)[:, 0].byte()
			target["masks"] = mask

		#resize bboxes
		bbox = target["boxes"]
		bbox = self.resize_boxes(bbox, (h, w), resized_img.shape[-2:])
		target["boxes"] = bbox

		#resize keypoints
		if "keypoints" in target:
			keypoints = target["keypoints"]
			keypoints = self.resize_keypoints(keypoints, (h, w), resized_img.shape[-2:])
			target["keypoints"] = keypoints
		

		return resized_img, target, resized_depth_img
