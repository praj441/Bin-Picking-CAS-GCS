...
data_path = root + 'ours_tase_high_clutter_data'
# data_path = root + '/data'
folder_path = data_path + '/depth_maps'
folder_path_2 = data_path + '/color_ims' 
out_final_path = data_path + '/out_DA_GCS_V1'
...
for idx in scan_names:
	...
	gcs_score_path = f"{out_final_path}/gcs_scores_{l-1}.npy"

	l=l+1 #index of masks starts from zero 
	mask = np.load(mask_path)
	scores = 100*np.load(score_path)
	gcs_scores = 100*np.load(gcs_score_path)
	# gcs_scores = np.where()
